...
#    >     -s <size>   Defines MIRRORMESH_INT  kind (4 or 8 usually)
#
#   Authors:
...
my $int      = 0;
my $fichier;
#my $format = "MIRRORMESH_INTEGER, PARAMETER :: %-30s = %d";
my $format = "#define %-30s %d";
my $formatbyval = "#define %-30s \%val(%d)";
my $definebyval = "#define MIRRORMESH_ARG_%-30s \%val(%d)\n";
my %opts;
...
sub Usage {
    ...
    print "  converts the file libmirrormesh*.h into a fortran header.\n";
...
sub printTab # ($chaine, $tabcount, $comm)
{
    ...
    if ($comm == 0)
    {
        if ($int != 0)
        {
            $chaine =~ s/MIRRORMESH_INTEGER,/INTEGER(KIND=$int),/g;
        }
        else
        {
            $chaine =~ s/MIRRORMESH_INTEGER,/INTEGER,/g;
        }
        if ($real != 0)
        {
    ...
    foreach my $line ( <APIc> )
    {
        if ($startcom == 0)
        {
            if ($startenum == 0)
            {
                ...
                elsif($line =~ /^\s*enum MIRRORMESH_arg/)
                {
                ...
                elsif ($line =~ /\#define MIRRORMESH_ARG_(\w*)\s+(.*)/)
                {
        ...
        else
        {
            ...
            {
                $chaine = sprintf("! %s\n", $line);
                if ($line =~ /Mirrormesh's constants/)
                {
                    ...
                    $chaine2 .= "!\n";
                    $chaine2 .= "!   Contains:\n";
                    $chaine2 .= "!     MIRRORMESH_DATA_PTR_T - Kind to use for POINTERS\n";
                   # $chaine2 .= "!     MIRRORMESH_REAL_KIND  - Kind to use for REAL\n";
                   # $chaine2 .= "!     MIRRORMESH_INT_KIND   - Kind to use for INT\n";
                    $chaine2 .= "!\n";
                    if ($real != 0)
                    {
                        $chaine2 .= "INTEGER, PARAMETER :: MIRRORMESH_REAL_KIND                = $real\n";
                    }
                    if ($int != 0)
                    {
                        $chaine2 .= "INTEGER, PARAMETER :: MIRRORMESH_INT_KIND                = $int\n";
                    }
                    $tabcount --;
