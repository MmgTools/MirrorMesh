 ...
 * headers for Fortran mirrormesh users.
 * Used by makefile
 ****************************************/
...
{
  char * header_f          = NULL;
  char * libmirrormesh_h       = NULL;
  char * libmirrormesh_include = NULL;
  char * genfort           = NULL;
  char * cmd               = NULL;
  FILE * file              = NULL;
  ...
  if (argc != 5)
    {
      fprintf(stderr, "usage : %s headerNameFortran.h headerNameC.h"
              " libmirrormeshtypesf_include_dir genfort.pl\n",argv[0]);
      return EXIT_FAILURE;
    }

  header_f          = argv[1];
  libmirrormesh_h       = argv[2];
  libmirrormesh_include = argv[3];
  genfort           = argv[4];
  ...
  fprintf(file,"! **\n");
  fprintf(file,"! ** This scripts recopy the macros of the libmirrormesh*.h file\n");
  ...
  fprintf(file,"! **\n");
  fprintf(file,"! ** See the \"libmirrormesh*.h\" file for a correct displaying of"
          " the documentation.\n");
  fprintf(file,"! */\n\n");

  /* Include libmirrormeshtypesf.h if needed. */
  if(strstr(libmirrormesh_h,"libmirrormeshtypes.h")==NULL) {
    fprintf(file,"#include \"%s/libmirrormeshtypesf.h\"\n\n",libmirrormesh_include);
  }
  else {
